    ...
    public class UIFoundationHandler : BaseCommandHandler
    {
        ...
        private object InspectUI(Dictionary<string, object> payload)
        {
            ...
            if (tmpText != null)
            {
                ...
                info["text"] = tmpText.text;
                info["fontSize"] = tmpText.fontSize;
            }
        ...
        private bool TryAddTextMeshPro(GameObject go, Dictionary<string, object> payload)
        {
            ...
            var tmp = Undo.AddComponent(go, tmpType) as TextMeshProUGUI;
            if (tmp == null) return false;

            tmp.text = GetString(payload, "text") ?? "New Text";
            tmp.fontSize = GetInt(payload, "fontSize", 14);
            tmp.alignment = TextAlignmentOptions.Center;

            if (payload.TryGetValue("color", out var colorObj) && colorObj is Dictionary<string, object> colorDict)
            {
                tmp.color = GetColorFromDict(colorDict, Color.black);
            }
            else
            {
        ...
        private bool TryAddTMPInputField(GameObject go, Dictionary<string, object> payload)
        {
            ...
            var tmpInput = Undo.AddComponent(go, tmpInputType) as TMP_InputField;
            ...
            var tmpText = Undo.AddComponent<TextMeshProUGUI>(textGo);
            tmpText.fontSize = GetInt(payload, "fontSize", 14);
            ...
            if (!string.IsNullOrEmpty(placeholder))
            {
                ...
                var placeholderText = Undo.AddComponent<TextMeshProUGUI>(placeholderGo);
                placeholderText.text = placeholder;
                placeholderText.fontSize = GetInt(payload, "fontSize", 14);
